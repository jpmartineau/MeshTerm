...
class _FakeBus:
    ...
    ``pair_reply`` is what ``Device1.Pair`` returns; ``asks`` makes BlueZ call the agent's
    ``RequestPasskey`` first, as a real Passkey Entry pairing does.
    """
...
def test_pair_registers_an_agent_answers_the_pin_and_trusts(bus) -> None:
    ...
    assert fake.calls[1:] == ["RegisterAgent", "Pair", "Set", "UnregisterAgent"]
